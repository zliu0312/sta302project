...
from the box plot, we think room type is the best catagorical data to use in our model 

```{r revenue log lm fit 1 with categorical, echo=FALSE}
df$property_type <- as.factor(df$room_type)
log_model_with_cat <- lm(
  log_total_revenue ~ age_of_exp + price + review_scores_value + review_scores_rating + accommodates + room_type, 
  data = df
)
